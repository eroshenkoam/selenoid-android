...
/usr/bin/xvfb-run -l -n "$DISPLAY" -s "-ac -screen 0 $SCREEN_RESOLUTION -noreset -listen tcp" /bin/sh -c "ANDROID_AVD_HOME=/root/.android/avd DISPLAY=:$DISPLAY /opt/android-sdk-linux/emulator/emulator $EMULATOR_ARGS -no-boot-anim -no-audio -no-jni -avd android8.0-1 -sdcard /sdcard.img -skin $SKIN -skindir /opt/android-sdk-linux/platforms/android-26/skins/ -verbose -gpu swiftshader_indirect -qemu -enable-kvm" &
